...
class TestPathlib(unittest.TestCase):
    ...
    def test_as_posix(self):
        self.assertTrue(Path("foo/bar").as_posix() == "foo/bar")
        self.assertTrue(Path("/abs/path").as_posix() == "/abs/path")

    def test_parents(self):
        p = Path("a/b/c")
        self.assertTrue(p.parents[0] == Path("a/b"))
        self.assertTrue(p.parents[1] == Path("a"))
        self.assertTrue(len(p.parents) >= 2)

    def test_relative_to(self):
        base = Path(self.tmp_path)
        child = base / "sub" / "file.py"
        child.parent.mkdir(parents=True)
        rel = child.relative_to(base)
        self.assertTrue(str(rel).replace("\\", "/") == "sub/file.py")

    def test_glob_yields_path(self):
        sub = Path(self.tmp_path) / "globdir"
        sub.mkdir()
        (sub / "a.txt").touch()
        results = list(sub.glob("*.txt"))
        self.assertTrue(len(results) == 1)
        self.assertTrue(isinstance(results[0], Path))
        self.assertTrue(results[0].name == "a.txt")

    def test_rglob_yields_path(self):
        sub = Path(self.tmp_path) / "rglobdir"
        nested = sub / "nested"
        nested.mkdir(parents=True)
        (nested / "b.py").touch()
        results = list(sub.rglob("*.py"))
        self.assertTrue(len(results) >= 1)
        self.assertTrue(isinstance(results[0], Path))
        self.assertTrue(results[0].name == "b.py")

    def test_resolve_returns_path(self):
        p = Path("foo")
        self.assertTrue(isinstance(p.resolve(), Path))
