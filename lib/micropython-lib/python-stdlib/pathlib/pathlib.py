...
class Path:
    ...
    def relative_to(self, other):
        other_path = Path(other).resolve()._path.rstrip(_SEP) or _SEP
        self_path = self.resolve()._path.rstrip(_SEP) or _SEP
        if self_path == other_path:
            return Path(".")
        prefix = other_path + _SEP
        if not self_path.startswith(prefix):
            raise ValueError("%s is not in the subpath of %s" % (self, other))
        return Path(self_path[len(prefix):])
