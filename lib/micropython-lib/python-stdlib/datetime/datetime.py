...
class tzinfo:
    ...
    def isoformat(self, dt):
        return self.utcoffset(dt)._fmt(18)
...
def _time(h, m, s, us, fold):
    if 0 <= h < 24 and 0 <= m < 60 and (0 <= s < 60) and (0 <= us < 1000000) and (fold == 0 or fold == 1) or (h == 0 and m == 0 and (s == 0) and (0 < us < 86400000000)):
...
def _t2iso(td, timespec, dt, tz) -> "str":
...
class time:
    ...
    def __repr__(self):
        return 'datetime.time(microsecond={}, tzinfo={}, fold={})'.format(self._td._us, repr(self._tz), self._fd)
    __str__ = isoformat
...
class datetime:
    ...
    def fromtimestamp(cls, ts, tz=None):
        ...
        if tz is None:
            ...
            s = (dt - datetime(*_t.localtime(ts - 86400)[:6]))._us // 1000000 - 86400
    ...
    def combine(cls, date, time, tzinfo=None):
        return cls(0, 0, date.toordinal(), 0, 0, 0, time._td._us, tzinfo or time._tz, fold=time._fd)
    ...
    def _mktime(self):
        ...
        def local(u):
            return (datetime(*_t.localtime(u)[:6]) - epoch)._us // 1000000
        epoch = datetime.EPOCH.replace(tzinfo=None)
        t, us = divmod((self - epoch)._us, 1000000)
        ts = None
        a = local(t) - t
        u1 = t - a
        t1 = local(u1)
    ...
    def __repr__(self):
        ...
        tz = repr(tz)
        return 'datetime.datetime({}, {}, {}, {}, {}, {}, {}, {}, fold={})'.format(Y, M, D, h, m, s, us, tz, fold)
